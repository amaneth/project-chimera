...
## Project Chimera Repository Rules

This repository is Project Chimera, an autonomous influencer system.

Before generating or modifying code, always read the relevant files in `specs/`.

Prime directive: NEVER generate code without checking `specs/` first.

Follow `.cursor/rules` for Project Chimera engineering behavior, including Java 21 conventions, Planner -> Worker -> Judge boundaries, JUnit 5 testing, MCP integration boundaries, and traceability expectations.

Do not remove or weaken Tenx MCP Sense trigger instructions in this file.
